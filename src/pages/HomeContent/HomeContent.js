    ...
    <HomeLayout bg={[bgIntro, bgStore, bgMain]} activeStep={activeStep}>
      <Grid container direction="column" className={classes.gridContainer}>
        ...
        <Grid item className={classes.gridItemIntro}>
          <Typography variant="h1" style={{ margin: "0.3em 0 0", fontSize: '2.375em' }}>{t('home#intro_welcome')}</Typography>
          <span style={{ fontSize: "24px" }} dangerouslySetInnerHTML={{ __html: t('home#intro_pitch') }} />
        </Grid>
        <div className={classes.bottomButton}>
          <Button forward onClick={handleNextButton} dangerouslySetInnerHTML={{ __html: t('home#intro_button') }} />
        </div>
      </Grid>
      <Grid container direction="column" className={classes.gridContainer}>
        <Grid item className={classes.gridItem}>
          <Typography variant="h1" style={{ marginBottom: '2em', padding: '0 2em' }}>{t('home#insertCode_title')}</Typography>
          <Input
      ...
      <Grid container direction="column">
        <Grid item className={classes.gridItem}>
          ...
          <div className={classes.bottomButton}>
            <Grid container style={{ marginBottom: '2.2em', padding: '0 3em', justifyContent: 'center' }}>
              <Grid item>
                <div style={{ fontSize: '2em', lineHeight: '1em' }}>{t('home#queue_length')}</div>
                <div style={{ color: '#401D7F', fontSize: '2.5em', lineHeight: '1em', fontWeight: 900 }}>7 {t('home#queue_people')}</div>
              </Grid>
            </Grid>
            <Button forward onClick={handleNextButton}>{t('home#queue_button')}</Button>
          </div>
        </Grid>
      </Grid>
      <Grid container direction="column">
        <Grid item className={classes.gridItem} style={{ paddingTop: '.8em' }}>
          <EmailNotification />
          <Typography variant="h4" style={{ padding: '0 2em' }} dangerouslySetInnerHTML={{ __html: t('home#notification_description') }} />
          <Input
            placeholder={t('home#notification_inputPlaceholder')}
            style={{ width: '100%', textAlign: 'center', fontWeight: 900, marginTop: '2.5em' }}
            value={userEmail}
            onChange={handleUserEmailChange}
          />
          <div className={classes.bottomButton}>
            <a onClick={handleTermsOnClick}>
              <div style={{ marginBottom: '1.75em', padding: '0 2em' }} dangerouslySetInnerHTML={{ __html: t('home#notification_terms') }} />
            </a>
      ...
      <Grid container direction="column">
        <Grid item className={classes.gridItem} style={{ paddingTop: '.8em' }}>
          ...
          <Grid container style={{ justifyContent: 'center', marginTop: '1.25em' }}>
            <Grid item style={{ position: 'relative' }}>
              <Ticket />
              <div style={{ position: 'absolute', top: '6em', left: 0, width: '100%', color: '#fff' }}>
          ...
          <Grid container justify="space-between" style={{ padding: '0 3em' }}>
            <Grid item>
              <div style={{ fontSize: '1.25em' }}>{t('home#ticket_currentQueue')}</div>
              <div style={{ fontSize: '2.8125em', fontWeight: 900 }}>019</div>
            </Grid>
            <Grid item>
              <div style={{ fontSize: '1.25em' }}>{t('home#ticket_length')}</div>
              <div style={{ fontSize: '2.8125em', fontWeight: 900 }}>9</div>
            </Grid>
          </Grid>
          <p style={{ fontSize: '1.375em', padding: '0 2em', marginTop: '.5em' }}>{t('home#ticket_notification')}</p>
        </Grid>
      </Grid>
    </HomeLayout>
  );
