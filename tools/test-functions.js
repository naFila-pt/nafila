...
createQueue({name: "My queue"}).then(async function(res) {
  ...
  console.log('addMeToQueue', await addMeToQueue({queueId, email:"email1@mail.com"}))
  ...
  console.log('removeMeFromQueue', await removeMeFromQueue({queueId,ticketId:added.data.id}))

  console.log('manuallyAddToQueue', await manuallyAddToQueue({queueId, phone:"9191919191-3"}))
  added = await addMeToQueue({queueId, email:"email4@mail.com"})
  console.log('addMeToQueue', added)
  console.log('manuallyAddToQueue', await manuallyAddToQueue({queueId, name:"Maria 5"}))
  console.log('removeMeFromQueue', await removeMeFromQueue({queueId,ticketId:added.data.id}))
  console.log('addMeToQueue', await addMeToQueue({queueId, email:"email6@mail.com"}))
  ...
  console.log('addMeToQueue', await addMeToQueue({queueId, email:"email7@mail.com"}))
  console.log('manuallyAddToQueue', await manuallyAddToQueue({queueId, phone:"9191919191-8"}))
